...
onload = function () {
	...
	document
		.getElementById('idGramado')
		.addEventListener('mousedown', marteloBaixo);
...
function mostraPontuacaoDe(display, valor) {
	let objCentena = document.getElementById(display).firstChild.nextSibling;
...
function marteloBaixo() {
	document.getElementById('idGramado').style.cursor =
		'url(./images/hammerDown.png), default';
}

function marteloCima() {
	document.getElementById('idGramado').style.cursor =
		'url(./images/hammer.png), crosshair';
}
